...
function _taggedHash(transaction, sighashType, inputNumber, sigversion, execdata) {
 ...
 */
function _signatureHash({ transaction, sighashType, inputIndex, sigversion, execdata }) {
  const ss = _taggedHash(transaction, sighashType, inputIndex, sigversion, execdata);
  // Return the SHA256 hash
  return ss.finalize();
};


function _sighashPreimage({ transaction, sighashType, inputIndex, sigversion, execdata }) {
  const ss = _taggedHash(transaction, sighashType, inputIndex, sigversion, execdata);
  // Return the SHA256 hash
  return ss.toBuffer();
};
 ...
 */
module.exports = {
  sighash: _signatureHash,
  sighashPreimage: _sighashPreimage,
  sign: sign,
  verify: verify
};
