...
function _signatureHash({ transaction, sighashType, inputNumber, sigversion, execdata }) {
  const ss = _taggedHash(transaction, sighashType, inputNumber, sigversion, execdata);
  // Return the SHA256 hash
  return ss.finalize();
};


function _sighashPreimage({ transaction, sighashType, inputNumber, sigversion, execdata }) {
  const ss = _taggedHash(transaction, sighashType, inputNumber, sigversion, execdata);
  // Return the SHA256 hash
  return ss.toBuffer();
};
